...
def get_tracks(url):
    try:
        ...
        filteredTracks = list(filter(lambda x: (("k3" in x and x['k3'] == "S") or ("k3" not in x)), tracks["data"]))
        mappedTracks = list(map(lambda x: Track(playtime=x["s"], artist=x["k1"], title=x["k2"]), filteredTracks))
        return mappedTracks

    except JSONDecodeError as e:
        logger.warning(f"Could not fetch from url {url}, response was {response.status_code}, body is not json decodable? (Response {response.content})?")
        return []
